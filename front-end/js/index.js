// Récupération de la liste des produits via une fonction
function fetchCamera() {
  let url = 'http://localhost:3000/api/cameras';
  return fetch(url).then((response) => response.json());
}

// Utilisation de la fonction pour récupérer la liste des produits
fetchCamera().then((cameraList) => {
  let affichage = '<div class="row">';
  // Affichage de la liste des produits via une boucle
  for (let camera of cameraList) {
    // HTML pour l'affichage des produits
